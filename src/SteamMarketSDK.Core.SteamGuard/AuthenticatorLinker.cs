...
public class AuthenticatorLinker
{
	...
	public async Task<LinkResult> AddAuthenticator()
	{
		...
		var addAuthenticatorResponseStr = await SteamWeb.PostRequest(
			"https://api.steampowered.com/ITwoFactorService/AddAuthenticator/v1/?access_token=" + Session.AccessToken,
	...
	public async Task<FinalizeResult> FinalizeAddAuthenticator(string smsCode)
	{
		...
		while (tries <= 10)
		{
			...
			using (var wc = new WebClient())
			{
				...
				var finalizeAuthenticatorResult = await wc.UploadValuesTaskAsync(
					new Uri(
						"https://api.steampowered.com/ITwoFactorService/FinalizeAddAuthenticator/v1/?access_token=" +
						Session.AccessToken), "POST", finalizeAuthenticatorValues);
	...
	{
		var setPhoneNumberLink =
			$"https://api.steampowered.com/IPhoneService/SetAccountPhoneNumber/v1?access_token={Session.AccessToken}";

		var setPhoneBody = new NameValueCollection
		{
			{ "phone_number", phoneNumber }, { "phone_country_code", countryCode }
		};
	...
	private async Task<string> GetUserCountry()
	{
		...
		var getUserCountryLink =
			$"https://api.steampowered.com/IUserAccountService/GetUserCountry/v1?access_token={Session.AccessToken}";
	...
	{
		var isWaitingLink =
			$"https://api.steampowered.com/IPhoneService/IsAccountWaitingForEmailConfirmation/v1?access_token={Session.AccessToken}";
	...
	private async Task SendPhoneVerificationCode()
	{
		var verificationLink =
			$"https://api.steampowered.com/IPhoneService/SendPhoneVerificationCode/v1?access_token={Session.AccessToken}";
...
{
	[JsonProperty("response")] public SetAccountPhoneNumberResponseResponse Response { get; set; }
}
...
{
	[JsonProperty("response")] public IsAccountWaitingForEmailConfirmationResponseResponse Response { get; set; }
}
...
internal class IsAccountWaitingForEmailConfirmationResponseResponse
{
	...
	[JsonProperty("seconds_to_wait")] public int SecondsToWait { get; set; }
}

internal class AddAuthenticatorResponse
{
	[JsonProperty("response")] public SteamGuardAccount Response { get; set; }
}

internal class FinalizeAuthenticatorResponse
{
	[JsonProperty("response")] public FinalizeAuthenticatorInternalResponse Response { get; set; }
	...
	internal class FinalizeAuthenticatorInternalResponse
	{
		...
		[JsonProperty("want_more")] public bool WantMore { get; set; }
	}
}

internal class GetUserCountryResponse
{
	[JsonProperty("response")] public GetUserCountryResponseResponse Response { get; set; }
}