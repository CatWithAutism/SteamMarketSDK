...
using SteamMarketSDK.IntegrationTests.Fixtures;
using Xunit;

namespace SteamMarketSDK.IntegrationTests.WebWrapper;
	...
	: IClassFixture<SteamHttpClientFixture>
{
	...
	private IInventoryWrapper InventoryWrapper { get; } = new InventoryWrapper(steamHttpClientFixture.SteamHttpClient);

	[Fact]
	public async Task CollectMarketAccountInfoTest()
	{
		var accountInfo = await MarketWrapper.GetAccountInfoAsync(CancellationToken.None);
	...
	{
		var inventory = await InventoryWrapper.GetInventory(SteamHttpClient.SteamId.ConvertToUInt64().ToString(), 730,
			2, "english", 100);
	...
	public async Task GetMarketHistoryTest()
	{
		...
		var marketHistory = await MarketWrapper.GetTradeHistoryAsync(offset, count, CancellationToken.None);
	...
	public async Task GetMyListingsHistoryTest()
	{
		const long offset = 0;
		const long count = 150;
		var marketHistory = await MarketWrapper.GetListingsAsync(offset, count, CancellationToken.None);
	...
	public async Task CreateAndCancelBuyOrderTest()
	{
		...
		const long offset = 0;
		const long count = 150;
		var myListings = await MarketWrapper.GetListingsAsync(offset, count, CancellationToken.None);
		...
		if (myListings.BuyOrders.Count > 0)
		{
			...
			{
				var cancelExistingOrder =
					await MarketWrapper.CancelBuyOrderAsync(existingOrder.BuyOrderId, CancellationToken.None);
	...
	public async Task GetItemNameIdTest()
	{
		...
		var itemId =
			await MarketWrapper.GetItemNameIdAsync(730, "P250 | Sand Dune (Field-Tested)", CancellationToken.None);
